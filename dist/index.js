...
async function plotSeries(seriesOrFrame, options = {}) {
  ...
  const plotPath = filename !== void 0 ? filename.includes("/") || filename.includes("\\") ? filename : join(defaultDir, filename) : join(defaultDir, "plot.html");
...
function defaultOutputDir2() {
  const documents = join2(homedir2(), "Documents");
  return existsSync2(documents) ? documents : homedir2();
}
...
async function sharpeplot(simulated, frontier, pointFrame, options = {}) {
  ...
  const plotPath = filename !== void 0 ? filename.includes("/") || filename.includes("\\") ? filename : join2(defaultDir, filename) : join2(defaultDir, "efficient-frontier.html");
  writeFileSync2(plotPath, html, "utf-8");
  if (autoOpen) {
    await open2(plotPath, { wait: false });
  }
  return plotPath;
}
export {
